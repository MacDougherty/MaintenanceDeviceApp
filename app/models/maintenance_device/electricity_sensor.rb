module MaintenanceDevice
  class ElectricitySensor < ApplicationRecord
    # The method(s) that correspond to device interfaces are: current port_number 
    
    belongs_to :maintenance_device
    attr_accessor :skip_extract
    after_commit :extract, unless: :skip_extract
    after_commit :channel_push
    
   	validates :current, inclusion: { in: 0..4095,
      ...
      if Apiotics.configuration.push == true
        ...
        interfaces.each do |k,v|
          ActionCable.server.broadcast "#{self.class.parent.to_s.underscore.downcase.gsub(" ", "_")}_channel",
            apiotics_instance: self.maintenance_device.apiotics_instance,
